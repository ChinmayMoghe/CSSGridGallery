...
access_key = a89a76c98ecf6b99da39fa2b0342a78041a9d558977c22442ca16db0351478a4;
...
const handleErrorResponse = (response)=> {
    if(!response.ok){
        convertResponseToJson(response).then((err)=>{console.log(err)}).catch((err)=>{console.log(err)});
        return;
    }
    return response;
}
...
const handleSuccessResponse = (response)=>{
   return convertResponseToJson(response).then((json)=>{
        let photos = json.map((photo)=>{return {'imgUrl':photo.urls.raw +`fm=webp&cs=tinysrgb&w=400&fit=max` ,'userName':photo.user.name,'userProfile':photo.user.links.html}});
...
function initApp(){
    fetch(`https://api.unsplash.com/photos/random?count=30&client_id=${atob("YTg5YTc2Yzk4ZWNmNmI5OWRhMzlmYTJiMDM0MmE3ODA0MWE5ZDU1ODk3N2MyMjQ0MmNhMTZkYjAzNTE0NzhhNA==")}
    `)
    .then((res)=>handleErrorResponse(res))
    .then((res)=>handleSuccessResponse(res))
    .then((smallphotoUrls)=>{createGridItems(smallphotoUrls)})
